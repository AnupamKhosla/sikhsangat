  ...
  ips.controller.register("forums.front.forum.flow", {
    ...
    toggleFilters: function (e) {
      ...
      $(document).trigger("updateTableURL", {
        forumId: _.uniq(_.values(this._ids)).join(","),
      });
    },
    isSelected: function (id) {
      return this.scope.find('[data-node-id="' + id + '"]').hasClass("cForumMiniList_selected")
        ? true
        : false;
    },
    ...
    parentUnselect: function (parentId) {
      ...
      var self = this;
      _.each(
        this.scope.find('[data-node-id="' + node.attr("data-parent-id") + '"]'),
        function (v, k) {
          var _cId = $(v).attr("data-node-id");
          if (_cId != parentId) {
  ...
  ips.controller.register("forums.front.forum.forumList", {
    ...
    toggleCategory: function (e) {
      ...
      if (!category.attr("data-hidden")) {
        ...
        category.removeClass("cForumRow_hidden").removeAttr("data-hidden");
      }
    },
    forumMarkedRead: function (e) {
      $(e.target)
        .closest(".cForumGrid")
        .removeClass("cForumGrid--unread")
        .removeClass("ipsDataItem_unread")
        .addClass("cForumGrid--read");
    },
  });
})(jQuery, _);
(function ($, _, undefined) {
