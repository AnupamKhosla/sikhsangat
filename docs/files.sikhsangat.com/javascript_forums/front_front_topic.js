...
(function ($, _, undefined) {
  ...
  ips.controller.register("forums.front.topic.answers", {
    ...
    rate: function (e) {
      ...
      this.ajaxObj = ips
        .getAjax()(clicked.attr("href"))
        .done(function (response) {
          Debug.log(response);
          voteCount.text(response.votes);
          self.scope
            .find(".ipsType_light")
            .text(ips.pluralize(ips.getString("votes_no_number"), response.votes));
        });
    },
    setStyles: function (setPositive, setNegative) {
      ...
      this.scope
        .toggleClass("cRatingColumn_up", setPositive)
        .toggleClass("cRatingColumn_down", setNegative);
