  ...
  ips.controller.register("core.front.core.articlePages", {
    ...
    stateChange: function () {
      ...
      if (
        ...
        state.data.controller != "article-" + this._articleID
      ) {
        return;
      }
      ...
      ips
        .getAjax()(
          ips.getSetting("baseURL") +
            "index.php?app=core&module=system&controller=notifications&do=button",
          {
            data: _.extend(
              {
              ...
              this.scope.attr("data-buttonType")
                ? {
                    button_type: this.scope.attr("data-buttonType"),
                  }
                : {},
            ),
          },
        ...
        .done(function (response) {
          ...
          if ($('input[data-toggle-id="auto_follow_toggle"]').length) {
            ...
            if (val == "false" && $('input[data-toggle-id="auto_follow_toggle"]').is(":checked")) {
              ...
              val == "true" &&
              !$('input[data-toggle-id="auto_follow_toggle"]').is(":checked")
            ) {
...
(function ($, _, undefined) {
  ...
  ips.controller.register("core.front.core.moderation", {
    ...
    editTitleMousedown: function (e) {
      ...
      this._editTimeout = setTimeout(function () {
        ...
        var inputNode = $("<input/>")
          ...
          .attr("data-role", "editTitleField")
          .val(anchor.text().trim());
    ...
    _modActionDelete: function (e) {
      ...
      e.preventDefault();
      var count =
        parseInt(this.scope.find('[data-role="moderation"]:checked').length) +
        parseInt(this.scope.find('[data-role="moderation"]:hidden').length);
      ips.ui.alert.show({
        type: "confirm",
        icon: "warn",
        message:
          count > 1
            ? ips.pluralize(ips.getString("delete_confirm_many"), count)
            : ips.getString("delete_confirm"),
        callbacks: {
          ok: function () {
            $(document).trigger("moderationSubmitted");
...
(function ($, _, undefined) {
  ...
  ips.controller.register("core.front.core.notifications", {
    ...
    setup: function () {
      this._timeout = null;
      if (
        ips.utils.notification.needsPermission() &&
        _.isUndefined(ips.utils.cookie.get("browserNotificationDismiss"))
      ) {
  ...
  ips.controller.register("core.front.core.poll", {
    ...
    submitPoll: function (e) {
      ...
      ips
        ...
        .fail(function () {
          form.attr("data-bypassAjax", true).submit();
        });
    },
  });
})(jQuery, _);
  ...
  ips.controller.register("core.front.core.pollEditor", {
    ...
    removeQuestion: function (e) {
      ...
      var removeQuestion = function () {
        question.replaceWith(
          '<div data-questionid="' + question.attr("data-questionid") + '"></div>',
        );
    ...
    _buildQuestion: function (data, qid) {
      ...
      if (_.isArray(data.choices) && data.choices.length) {
        for (var i = 0; i < data.choices.length; i++) {
          choices.push(
            this._getChoiceHTML(i + 1, qid, data.choices[i].title, data.choices[i].count),
          );
        }
      ...
      this.scope.find('[data-role="pollContainer"]').append(
        ips.templates.render("core.pollEditor.question", {
          pollName: this._name,
    ...
    _addQuestion: function (qid) {
      ...
      this.scope.find('[data-role="pollContainer"]').append(
        ips.templates.render("core.pollEditor.question", {
          pollName: this._name,
          questionTitle: ips.getString("questionTitle", {
            id: qid,
          }),
          questionID: qid,
    ...
    _checkQuestionButton: function () {
      var questions = this.scope.find("[data-questionID]");
      this.scope
        .find('[data-action="addQuestion"]')
        .toggleClass("ipsButton_disabled ipsFaded", questions.length >= this._maxQuestions);
    },
    _checkChoiceButton: function (questionScope) {
      var choices = questionScope.find("[data-choiceID]");
      questionScope
        .find('[data-action="addChoice"]')
        .toggleClass("ipsButton_disabled ipsFaded", choices.length >= this._maxChoices);
      questionScope
        .find('[data-choiceID] [data-action="removeChoice"]')
        .toggleClass("ipsButton_disabled ipsFaded", choices.length === 2);
    },
  });
})(jQuery, _);
(function ($, _, undefined) {
  ...
  ips.controller.register("core.front.core.profileCompletion", {
    ...
    dismissProfile: function (e) {
      ...
      ips
        .getAjax()(
          ips.getSetting("baseURL") +
            "index.php?app=core&module=system&controller=settings&do=dismissProfile",
        )
        .done(function (response) {
          self.scope.animate(
            {
...
(function ($, _, undefined) {
  ...
  ips.controller.register("core.front.core.statusFeedWidget", {
    ...
    focusNewStatus: function (e) {
      ...
      ips
        .getAjax()(
          ips.getSetting("baseURL") + "index.php?app=core&module=status&controller=ajaxcreate",
        )
        .done(function (response) {
    ...
    editorReady: function (e, data) {
      ...
      this.scope
        .find('[data-role="statusDummy"]')
        .hide()
        .find(".ipsComposeArea_dummy")
        .text(ips.getString("whatsOnYourMind"));
      try {
    ...
    submitNewStatus: function (e) {
      ...
      form
        .find('button[type="submit"]')
        .prop("disabled", true)
        .text(ips.getString("updatingStatus"));
      ips
        ...
        .always(function () {
          form
            .find('button[type="submit"]')
            .prop("disabled", false)
            .text(ips.getString("submitStatus"));
        });
    },
  });
})(jQuery, _);
