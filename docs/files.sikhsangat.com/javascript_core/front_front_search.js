(function ($, _, undefined) {
  ...
  ips.controller.register("core.front.search.filters", {
    ...
    resultsDone: function (e, data) {
      ...
      this.scope
        .find('[data-action="searchAgain"]')
        .removeClass("ipsHide ipsButton_disabled")
        .show();
    ...
    resultsLoading: function (e, data) {
      ...
      searchButton
        .prop("disabled", true)
        .attr("data-originalText", searchButton.text())
        .text(ips.getString("searchFetchingResults"));
    },
    ...
    submitForm: function (e) {
      ...
      var tagExists =
        this.scope.find("#elInput_tags").length &&
        this.scope.find("#elTab_searchContent").hasClass("ipsTabs_activeItem");
      if (tagExists) {
          ...
          ips.ui.alert.show({
            ...
            message:
              !searchTerm && !tagExists
                ? ips.getString("searchRequiresTerm")
                : ips.getString("searchRequiresTermTags"),
            icon: "info",
            callbacks: {
              ok: function () {
                setTimeout(function () {
...
(function ($, _, undefined) {
  ...
  ips.controller.register("core.front.search.main", {
    ...
    stateChange: function () {
      ...
      if (
        (!state.data.controller || state.data.controller != "core.front.search.main") &&
    ...
    _getUrlFromData: function (data) {
      ...
      if (data["type"] == "core_members") {
        if (!_.isUndefined(data["joinedDate"])) {
          if (data["joinedDate"] !== "custom") {
            ...
            if (!_.isUndefined(data["joinedDateCustom[start]"])) {
              params.push(
                "start_after=" +
                  encodeURIComponent(new Date(data["joinedDateCustom[start]"]).getTime() / 1000),
              );
            }
            if (!_.isUndefined(data["joinedDateCustom[end]"])) {
              params.push(
                "start_before=" +
                  encodeURIComponent(new Date(data["joinedDateCustom[end]"]).getTime() / 1000),
              );
            }
          }
        }
        ...
        if (!_.isUndefined(data["eitherTermsOrTags"])) {
          if (
            !_.isUndefined(data["q"]) &&
            data["q"].trim() !== "" &&
            !_.isUndefined(data["tags"]) &&
            data["tags"].trim() !== ""
          ) {
        ...
        if (
          !_.isUndefined(data["search_and_or"]) &&
          (data["search_and_or"] == "or" || data["search_and_or"] == "and")
        ) {
        ...
        _.each(
          [
            ["startDateCustom[start]", "start_after"],
            ["startDateCustom[end]", "start_before"],
            ["updatedDateCustom[start]", "updated_after"],
            ["updatedDateCustom[end]", "updated_before"],
          ],
          function (val) {
            var thisType = val[0].indexOf("startDate") != -1 ? "startDate" : "updatedDate";
            if (!_.isUndefined(data[val[0]]) && !datesSet[thisType]) {
              if (
                (val[0] == "startDateCustom[start]" || val[0] == "startDateCustom[end]") &&
                !_.isUndefined(data["startDate"]) &&
                data["startDate"] == "any"
              ) {
              } else if (
                (val[0] == "updatedDateCustom[start]" || val[0] == "updatedDateCustom[end]") &&
                !_.isUndefined(data["updatedDate"]) &&
                data["updatedDate"] == "any"
              ) {
              } else {
                params.push(
                  val[1] +
                    "=" +
                    encodeURIComponent(
                      ips.utils.time.getDateFromInput($('[name="' + val[0] + '"]')).getTime() /
                        1000,
                    ),
                );
              }
    ...
    _loadResults: function (url) {
      ...
      ips
        ...
        .done(function (response) {
          ...
          var newItems = self.scope
            .find('[data-role="resultsArea"] [data-role="activityItem"]')
            .css({
              opacity: "0",
            });
          var delay = 100;
    ...
    _addCSS: function (css) {
      ...
      if (css && css.length) {
        for (var i = 0; i < css.length; i++) {
          head.append(
            $("<link/>").attr("href", css[i]).attr("type", "text/css").attr("rel", "stylesheet"),
          );
        }
      }
    },
...
(function ($, _, undefined) {
  ...
  ips.controller.register("core.front.search.results", {
    ...
    _findWords: function (result) {
      ...
      var searchBack =
        firstMatch - this._resultLength / 2 < 0 ? 0 : firstMatch - this._resultLength / 2;
      if (!foundMatches) {
        startPoint = 0;
      } else {
      ...
      if (
        startPoint + this._resultLength < text.length ||
        (!foundMatches && text.length > this._resultLength)
      ) {
        finalSnippet = finalSnippet + "...";
      }
            ...
            .each(function () {
              $(this).replaceWith(
                _.escape(
                  XRegExp.replace(
                    $(this).text(),
                    new RegExp("(\\b|\\s|^)(" + term + "\\w*)(\\b|\\s|$)", "ig"),
                    '<mark class="ipsMatch' + (index + 1) + '">' + "$2 " + "</mark>",
                  ),
                )
                  .replace(
                    new RegExp("&lt;mark class=&quot;ipsMatch" + (index + 1) + "&quot;&gt;", "ig"),
                    "<mark class='ipsMatch" + (index + 1) + "'>",
                  )
                  .replace(new RegExp("&lt;/mark&gt;", "ig"), "</mark>"),
              );
            });
        });
      });
