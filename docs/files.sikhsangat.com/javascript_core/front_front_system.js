ips.templates.set(
  "follow.frequency",
  " {{#hasNotifications}}  <i class='fa fa-bell'></i> {{/hasNotifications}} {{^hasNotifications}}  <i class='fa fa-bell-slash-o'></i> {{/hasNotifications}} {{text}}",
);
  ...
  ips.controller.register("core.front.system.manageFollowed", {
    ...
    followingItemChange: function (e, data) {
      ...
      if (data.notificationType) {
        this.scope.find('[data-role="followFrequency"]').html(
          ips.templates.render("follow.frequency", {
...
(function ($, _, undefined) {
  ...
  ips.controller.register("core.front.system.metaTagEditor", {
    ...
    restoreDefaultMeta: function (e) {
      ...
      var copy = this.scope
        .find('[data-role="metaTemplate"]')
        .clone()
        .attr("data-role", "metaTagRow")
        .hide();
      ...
      if (this.scope.find('input[name="defaultMetaTag[' + tag + ']"]')) {
        copy
          .find('[name="meta_tag_content[]"]')
          .val(this.scope.find('input[name="defaultMetaTag[' + tag + ']"]').val());
      }
    ...
    removeDefaultMeta: function (e) {
      if ($(e.currentTarget).siblings("select").first().val() == "other") {
        var name = $(e.currentTarget)
          .closest("ul")
          .find('input[name="meta_tag_name_other[]"]')
          .val();
      } else {
      ...
      var copy = this.scope
        .find('[data-role="metaDefaultDeletedTemplate"]')
        .clone()
        .attr("data-role", "metaTagRow")
        .hide();
...
(function ($, _, undefined) {
  ...
  ips.controller.register("core.front.system.notificationSettings", {
              ...
              ips
                .getAjax()(
                  ips.getSetting("baseURL") +
                    "index.php?app=core&module=system&controller=notifications&do=verifySubscription",
                  {
                    type: "post",
                    data: {
                      key,
    ...
    subscribePending: function (e, data) {
      this.scope
        .find('[data-action="enablePush"]')
        .html(ips.templates.render("core.notifications.pending"))
        .show();
    },
    subscribeSuccess: function (e, data) {
      this.scope
        .find('[data-action="enablePush"]')
        .html(ips.templates.render("core.notifications.success"))
        .show();
    },
    subscribeFail: function (e, data) {
      this.scope
        .find('[data-action="enablePush"]')
        .html(ips.templates.render("core.notifications.fail"))
        .show();
    },
    ...
    showNotificationSettings: function (e) {
      ...
      this.scope
        .find(".cNotificationTypes__row--selected")
        .removeClass("cNotificationTypes__row--selected");
      ...
      target
        .find(".cNotificationSettings_expand")
        .addClass("ipsLoading ipsLoading_tiny")
        .find("i")
        .addClass("ipsHide");
      ips
...
(function ($, _, undefined) {
  ...
  ips.controller.register("core.front.system.register", {
    ...
    convertExistingErrors: function () {
      var fields = this.scope.find(
        "#elInput_username, #elInput_password, #elInput_password_confirm",
      );
      var self = this;
      fields.each(function () {
        ...
        wrapper
          .find('[data-role="validationCheck"]')
          .show()
          .html(
    ...
    changePassword: function (e) {
      ...
      if (this.passwordField.val().length > 2 || e.type != "keyup") {
        this.timers["password"] = setTimeout(
          _.bind(this._doPasswordCheck, this, this.passwordField),
          200,
        );
      } else {
    ...
    confirmPassword: function (e) {
      ...
      if (
        ...
        this.passwordField.val() === this.confirmPasswordField.val()
      ) {
    ...
    _doCheck: function (field) {
      ...
      this.ajax(
        ips.getSetting("baseURL") + "?app=core&module=system&controller=ajax&do=usernameExists",
        {
          dataType: "json",
          data: {
