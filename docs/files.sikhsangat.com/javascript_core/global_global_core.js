  ...
  ips.controller.register("core.global.core.authyOneTouch", {
    initialize: function () {
      ...
      setInterval(function () {
        ips
          .getAjax()(scope.closest("form").attr("action"), {
            data: {
...
(function ($, _, undefined) {
  ...
  ips.controller.register("core.global.core.coverPhoto", {
    ...
    removePhoto: function (data) {
      ...
      ips.ui.alert.show({
        ...
        callbacks: {
          ok: function () {
            ips
              .getAjax()($(data.originalEvent.target).attr("href") + "&wasConfirmed=1")
              .done(function () {
    ...
    savePosition: function (e) {
      ...
      ips
        .getAjax()(this.scope.attr("data-url") + "&do=coverPhotoPosition" + "&offset=" + newOffset)
        .fail(function (err) {
    ...
    positionPhoto: function (e) {
      ...
      this.scope
        .find(".ipsCoverPhoto_container")
        .append(ips.templates.render("core.coverPhoto.controls"));
      this._showTooltip();
...
(function ($, _, undefined) {
  ...
  ips.controller.register("core.global.core.datetime", {
    ...
    setup: function () {
      ...
      $(this.scope).text(
        ips.utils.time.formatTime(new Date($(this.scope).attr("data-time")), formatObject),
      );
    },
  });
})(jQuery, _);
...
(function ($, _, undefined) {
  ...
  ips.controller.register("core.global.core.genericTable", {
    ...
    stateChange: function () {
      ...
      if (
        !_.isUndefined(state.data.sortby) &&
        !_.isUndefined(state.data.sortdirection) &&
        (state.data.sortby != this._urlParams.sortby ||
          state.data.sortdirection != this._urlParams.sortdirection)
      ) {
        this._updateSort({
    ...
    _getResults: function () {
      var self = this;
      ips
        .getAjax()(this._baseURL + this._getURL() + "&" + this.scope.attr("data-resort") + "=1", {
          dataType: "json",
          showLoading: true,
        })
        .done(function (response) {
    ...
    _getSortValue: function () {
      var sortBy = this.scope.find(
        '[data-role="table"] thead .ipsTable_sortable.ipsTable_sortableActive',
      );
      var sortOrder = "desc";
    ...
    showWait: function () {
      ...
      var interval = setInterval(
        function () {
          ...
          progressBar.css(
            "width",
            100 - (100 / (waitUntil - start)) * (waitUntil - Date.now()) + "%",
          );
        }.bind(this),
        100,
      );
...
(function ($, _, undefined) {
  ...
  ips.controller.register("core.global.core.licenseRenewal", {
    ...
    close: function (e) {
      if (
        $("body")
          .find('[data-role="licenseRenewal"]')
          .find("input[type=checkbox][name=hideRenewalNotice]")
          .is(":checked")
      ) {
        var notification = $(this.scope).closest(".cNotification,.cAcpNotificationBanner");
        ips
          .getAjax()($(this.scope).find('[data-action="notNow"]').attr("href"))
          .done(function (response) {
            ...
            if (!notification.closest(".cNotificationList").children().count) {
              ips.utils.anim.go(
                "fadeIn",
                notification
                  .closest(".cNotificationList")
                  .find('[data-role="empty"]')
                  .removeClass("ipsHide"),
              );
            }
...
(function ($, _, undefined) {
  ...
  ips.controller.register("core.global.core.notificationList", {
    ...
    dismiss: function (e) {
      e.preventDefault();
      var notification = $(e.target).closest(
        '[data-role="notificationBlock"],.cAcpNotificationBanner',
      );
      ips.ui.alert.show({
        type: "confirm",
        ...
        callbacks: {
          ok: function () {
            ips
              .getAjax()(notification.find('[data-action="dismiss"]').attr("href"))
              .done(function (response) {
                ...
                ips.utils.anim.go("fadeOut", notification).done(function () {
                  if (
                    !notification
                      .closest(".cNotificationList")
                      .children('[data-role="notificationBlock"]:not(.cNotification_hidden)').length
                  ) {
                    ips.utils.anim.go(
                      "fadeIn",
                      notification
...
(function ($, _, undefined) {
  ...
  ips.controller.register("core.global.core.optionalAutocomplete", {
    ...
    setup: function () {
      ...
      this.scope.html(div);
      this.scope.append(
        ips.templates.render("core.autocomplete.optional", {
      ...
      if (
        this._autoComplete.attr("data-ipsAutocomplete-freeChoice") &&
        this._autoComplete.attr("data-ipsAutocomplete-freeChoice") == "false"
      ) {
  ...
  ips.controller.register("core.global.core.prefixedAutocomplete", {
    ...
    autoCompleteReady: function (e, data) {
      ...
      if (this._prefixValue && tokens.length) {
        ...
        this._prefixButton
          .find("span")
          .html(this._getPrefixText(_.escape(this._prefixValue.val())));
...
(function ($, _, undefined) {
  ...
  ips.controller.register("core.global.core.table", {
    ...
    stateChange: function () {
      var state = History.getState();
      if (
        (_.isUndefined(state.data.controller) || state.data.controller != this.controllerID) &&
        this._initialURL != state.url
      ) {
        return;
      }
    ...
    _cleanUpBaseURL: function () {
      ...
      if (urlObj.file == "index.php") {
        _.each(params, function (val, key) {
          if (key.startsWith("/")) {
            self._baseURL += encodeURIComponent(key).replace(/%2f/gi, "/");
            delete params[key];
          }
        });
        this._baseURL += "&";
    ...
    _handleStateChanges: function (state) {
      ...
      if (
        !_.isUndefined(state.data[this._pageParam]) &&
        state.data[this._pageParam] != this._urlParams[this._pageParam]
      ) {
    ...
    _getResults: function (forceURL) {
      ...
      if (forceURL) {
        ...
        if (this._seoPagination) {
          url = ips.utils.url.pageParamToPath(
            url,
            this._pageParam,
            this._urlParams[this._pageParam],
          );
        }
      }
    ...
    rowClick: function (e) {
      ...
      var link = $(e.currentTarget)
        .find("[data-ipscontrolstrip]")
        .parent()
        .find(
          '[data-controlStrip-action="' + $(e.currentTarget).attr("data-tableClickTarget") + '"]',
        );
      ...
      scope.find('[data-role="serverInput"]').keyup(function () {
        ...
        if (matches && (matches[1] || matches[3] || matches[8] || matches[10])) {
          if (matches[2]) {
            ...
            scope
              .find('[data-role="portToggle"][value="' + matches[2] + '"]')
              .prop("checked", true);
          }
          if (matches[3]) {
            if (matches[4]) {
