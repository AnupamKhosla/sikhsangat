(function ($, _, undefined) {
  ...
  ips.controller.mixin("contentListing", "core.global.core.table", true, function () {
    ...
    this.frontPaginationClicked = function () {
      ...
      var elemPosition = ips.utils.position.getElemPosition(
        wrappingDialog.length ? wrappingDialog : this.scope,
      );
      $("html, body").animate({
...
(function ($, _, undefined) {
  ...
  ips.controller.register("core.front.core.app", {
    ...
    setup: function () {
      if (
        parseInt(ipsSettings["memberID"]) == 0 &&
        ips.utils.cookie.get("loggedIn") !== undefined &&
        parseInt(ips.utils.cookie.get("loggedIn")) > 0
      ) {
        if (!window.location.href.match(/ct=\d{10}/)) {
          var ts = Math.round(new Date().getTime() / 1000);
          window.location.href += window.location.href.match(/\?/) ? "&ct=" + ts : "?ct=" + ts;
        }
      }
    ...
    _fixMissingLazyLoadItems: function (container) {
      ...
      content.each(function () {
        var closest = $(this).closest(
          '[data-ipsLazyLoad], [data-controller^="core.front.core.lightboxedImages"]',
        );
        if (!closest.length) {
...
(function ($, _, undefined) {
  ...
  ips.controller.register("core.front.core.autoSizeIframe", {
    ...
    _startReadyTimeout: function () {
      this._readyTimeout = setInterval(
        _.bind(function () {
    ...
    _getBorderAdjustment: function () {
      this._border.vertical =
        parseInt(this.scope.css("border-top-width")) +
        parseInt(this.scope.css("border-bottom-width"));
      this._border.horizontal =
        parseInt(this.scope.css("border-left-width")) +
        parseInt(this.scope.css("border-right-width"));
    },
    ...
    dims: function (data) {
      if (
        parseInt(this._lastDims.height) !== parseInt(data.height) ||
        this._lastDims.width !== data.width
      ) {
        this.scope.css({
          height: parseInt(data.height) + this._border.vertical + "px",
          maxWidth:
            data.width.toString().indexOf("%") == -1
              ? parseInt(data.width) + this._border.horizontal + "px"
              : "100%",
        });
        this._lastDims.height = data.height;
        this._lastDims.width = data.width;
...
(function ($, _, undefined) {
  ...
  ips.controller.register("core.front.core.comment", {
    ...
    initialize: function () {
      ...
      this.on(
        "mouseup touchend touchcancel selectionchange",
      ...
      this.on(
        document,
        "getEditFormLoading.comment saveEditCommentLoading.comment " +
          "deleteCommentLoading.comment",
        this.commentLoading,
      );
      this.on(
        document,
        "getEditFormDone.comment saveEditCommentDone.comment " + "deleteCommentDone.comment",
        this.commentDone,
      );
    ...
    recommendComment: function (e, data) {
      var commentHtml = $("<div>" + data.response.comment + "</div>")
        .find('[data-controller="core.front.core.comment"]')
        .html();
    ...
    unrecommendCommentDone: function (e, data) {
      ...
      var commentHtml = $("<div>" + data.comment + "</div>")
        .find('[data-controller="core.front.core.comment"]')
        .html();
      this.scope
        .html(commentHtml)
    ...
    _checkQuoteStatus: function () {
      var selectedText = ips.utils.selection.getSelectedText(
        '[data-role="commentContent"]',
        this.scope.find('[data-role="commentContent"]').parent(),
      );
    ...
    _showQuoteTooltip: function () {
      ...
      if (!tooltip.length) {
        this.scope.append(
          ips.templates.render("core.selection.quote", {
            direction: "bottom",
          }),
        );
      ...
      if (range.type === "outside") {
        ...
        position.left =
          boundingBox.left + boundingBox.width / 2 + $(window).scrollLeft() - offset.left;
      ...
      var leftAdjustment = ips.utils.events.isTouchDevice()
        ? tooltipSize.width
        : tooltipSize.width / 2;
      tooltip.css({
        position: "absolute",
    ...
    setMultiQuote: function (e, data) {
      ...
      if (this.scope.is(selector)) {
        if (!_.isNull(e) && e.type == "setMultiQuoteEnabled") {
          this.scope
            .find('[data-action="multiQuoteComment"]')
    ...
    quoteComment: function (e) {
      ...
      var html = this._prepareQuote(
        $("<div/>").html(this.scope.find('[data-role="commentContent"]').html()),
      );
      this.trigger("quoteComment.comment", {
        userid: this._quoteData.userid,
        username: this._quoteData.username,
    ...
    submitEdit: function (e) {
      ...
      if (typeof CKEDITOR.instances["comment_value"] !== "undefined") {
        var postBody = CKEDITOR.instances["comment_value"]
          .editable()
          .getData()
          .replace(/&nbsp;/g, "")
          .trim();
    ...
    saveEditCommentDone: function (e, data) {
      ...
      this.scope
        ...
        .replaceWith($("<div>" + data.response + "</div>").find('[data-role="commentContent"]'));
    ...
    approveCommentDone: function (e, data) {
      ...
      var commentHtml = $("<div>" + data.response + "</div>")
        .find('[data-controller="core.front.core.comment"]')
        .html();
      this.scope
        .html(commentHtml)
    ...
    deleteCommentDone: function (e, data) {
      ...
      if (deleteLink.attr("data-updateOnDelete")) {
        $(deleteLink.attr("data-updateOnDelete")).text(
          parseInt($(deleteLink.attr("data-updateOnDelete")).text()) - 1,
        );
      }
    ...
    _prepareQuote: function (html) {
      if (
        html.find("blockquote.ipsQuote") &&
        html.find("blockquote.ipsQuote").parent() &&
        html.find("blockquote.ipsQuote").parent().get(0) &&
        html.find("blockquote.ipsQuote").parent().get(0).tagName == "DIV" &&
        html.find("blockquote.ipsQuote").siblings().length == 0
      ) {
      ...
      html
        .find(".ipsStyle_spoilerFancy,.ipsStyle_spoiler")
        .replaceWith(ips.templates.render("core.posts.quotedSpoiler"));
...
(function ($, _, undefined) {
  ...
  ips.controller.register("core.front.core.commentFeed", {
    ...
    stateChange: function () {
      ...
      if (
        _.isUndefined(state.data.controller) ||
        state.data.controller != this.controllerID ||
        state.data.feedID != this._commentFeedID
      ) {
    ...
    _getResultsDone: function (response) {
      var tmpElement = $("<div>" + response + "</div>").find(
        '[data-feedID="' + this.scope.attr("data-feedID") + '"]',
      );
    ...
    _updateURL: function (newParams) {
      ...
      History.pushState(tmpStateData, document.title, newUrl);
    },
    _getURL: function () {
      var tmpUrlParams = {};
      for (var i in this._urlParams) {
        if (
          ...
          i != "feedID" &&
          i != "bypassState" &&
          (i != "page" || (i == "page" && this._urlParams[i] > 1))
        ) {
    ...
    _ignoreAllSignatures: function () {
      ...
      var url =
        ...
        "index.php?app=core&module=system&controller=settings&do=toggleSigs";
    ...
    _ignoreSingleSignature: function (memberID) {
      ...
      var url =
        ips.getSetting("baseURL") +
        "index.php?app=core&module=system&controller=ignore&do=ignoreType&type=signatures";
      var signatures = this.scope
        .find('[data-role="memberSignature"]')
        .find('[data-memberID="' + memberID + '"]')
        .closest('[data-role="memberSignature"]');
      signatures.slideUp();
      ips
        .getAjax()(url, {
          data: {
    ...
    quoteComment: function (e, data) {
      ips.ui.editor.getObjWithInit(
        this.scope.find('[data-role="replyArea"] [data-ipsEditor]'),
        function (editor) {
    ...
    pollForNewReplies: function () {
      ...
      this._pollAjax(replyForm.attr("action"), {
        dataType: "json",
        data:
          "do=checkForNewReplies&type=count&lastSeenID=" +
          lastSeenId +
          "&csrfKey=" +
    ...
    _buildFlashMsg: function (response) {
      ...
      if (parseInt(response.count) > spaceForMore) {
        ...
        !_.isUndefined(response.photo) &&
        !_.isUndefined(response.name)
      ) {
      ...
      if (
        $("#elFlashMessage").is(":visible") &&
        $("#elFlashMessage").find('[data-role="newPostNotification"]').length
      ) {
    ...
    _buildNotifications: function (response) {
      ...
      if (
        _.isUndefined(hiddenProp) ||
        !document[hiddenProp] ||
        !ips.utils.notification.hasPermission()
      ) {
        return;
      }
      var notifyData = {
      ...
      if (
        ...
        notifyData = _.extend(notifyData, {
          ...
          body: ips.pluralize(
            ips.getString("notificationNewPostMultipleBody", {
              title: response.title,
            }),
            [response.count],
          ),
    ...
    _importNewReplies: function () {
      ...
      var self = this;
      ips
        .getAjax()(form.attr("action"), {
          data:
            "do=checkForNewReplies&type=fetch&lastSeenID=" +
            _lastSeenID +
            "&showing=" +
            commentsOnThisPage.length +
            "&csrfKey=" +
            ips.getSetting("csrfKey"),
          type: "post",
        })
    ...
    _clearNotifications: function () {
      ...
      if ($("#elFlashMessage").find('[data-role="newPostNotification"]').length) {
        $("#elFlashMessage")
          .find('[data-role="newPostNotification"]')
          .trigger("closeFlashMsg.flashMsg");
      }
    },
      ...
      ips
        ...
        .done(function (response) {
          if (response.type == "error") {
            ...
            ips.ui.editor
              .getObj(self.scope.find('[data-role="replyArea"] [data-ipsEditor]'))
              .reset();
        ...
        .fail(function (jqXHR, textStatus, errorThrown) {
          if (Debug.isEnabled()) {
            ...
            form.attr(
              "action",
              form.attr("action") +
                (!form.attr("action").match(/\?/) ? "?failedReply=1" : "&failedReply=1"),
            );
            form.submit();
          }
        })
    ...
    addToCommentFeed: function (e, data) {
      ...
      if (
        !_.isUndefined(data.scrollToItem) &&
        data.scrollToItem &&
        (newItemTop < windowScroll || newItemTop > windowScroll + viewHeight)
      ) {
        $("html, body").animate(
          {
    ...
    _updateCount: function (newTotal) {
      if (this.scope.find('[data-role="comment_count"]')) {
        ...
        if (this.scope.find('[data-role="comment_count"]').attr("data-commentCountString")) {
          langString = this.scope
            .find('[data-role="comment_count"]')
            .attr("data-commentCountString");
        }
        this.scope
          .find('[data-role="comment_count"]')
          .text(ips.pluralize(ips.getString(langString), newTotal));
      }
    },
    ...
    _buildMultiQuote: function (count) {
      ...
      if (!quoterElem.length && count) {
        ips.getContainer().append(
          ips.templates.render("core.posts.multiQuoter", {
        ...
        quoterElem
          .find('[data-role="quotingTotal"]')
          .text(ips.pluralize(ips.getString("multiquote_count_plural"), [count]));
    ...
    _getLastSeenID: function (commentsOnThisPage) {
      ...
      if (
        commentFeed.attr("data-topicAnswerID") &&
        parseInt(commentFeed.attr("data-topicAnswerID")) > max
      ) {
...
(function ($, _, undefined) {
  ...
  ips.controller.register("core.front.core.commentsWrapper", {
    ...
    deletedComment: function (e, data) {
      ...
      this._updateCount(
        $(e.target).closest("[data-commentsType]").attr("data-commentsType"),
        newTotal,
      );
    },
...
(function ($, _, undefined) {
  ...
  ips.controller.register("core.front.core.ignoredComments", {
    ...
    _stopIgnoringFromComment: function (e, data) {
      ...
      var url =
        ips.getSetting("baseURL") +
        "index.php?app=core&module=system&controller=ignore&do=ignoreType&type=topics&off=1";
      ips
        .getAjax()(url, {
          data: {
        ...
        .fail(function () {
          window.location =
            ips.getSetting("baseURL") +
            "index.php?app=core&module=system&controller=ignore&do=ignoreType&off=1type=topics&member_id=" +
            userID;
        });
    },
  });
  ...
  ips.controller.register("core.front.core.instantNotifications", {
    ...
    setup: function () {
      if (!ips.utils.db.isEnabled() || !_.isFunction(JSON.parse)) {
        Debug.warn(
          "Sorry, your browser doesn't support localStorage or JSON so we can't load instant notifications for you.",
        );
        return;
      }
    ...
    _doInitialCheck: function () {
      ...
      if (
        (this._messagesEnabled && counts.messages > storage.messages) ||
        counts.notifications > storage.notifications
      ) {
        if (this._debugPolling) {
          Debug.log(
            "Notifications: bubbles reporting higher counts for notifications or messages.",
          );
        }
        var dataToSend = {
    ...
    _checkNotifications: function () {
      ...
      if (
        storage &&
        _.isObject(storage) &&
        parseInt(storage.timestamp) > timestamp - (currentTimeout - 1) * 1000
      ) {
    ...
    _doAjaxRequest: function (dataToSend) {
      ...
      var url =
        ips.getSetting("baseURL") +
        "?app=core&module=system&controller=ajax&do=instantNotifications";
      if (this._debugPolling) {
    ...
    _handleResponse: function (response) {
      try {
        ...
        if (response.notifications.count > counts.notifications && this._debugPolling) {
          Debug.log(
            "Notifications: I'm the winner! I found there's " +
        ...
        if (response.notifications.data.length) {
          this._showNotification(
            this._buildNotifyData(response.notifications.data, "notification"),
            "notification",
          );
        }
        if (response.messages.data.length) {
          this._showNotification(
            this._buildNotifyData(response.messages.data, "message"),
            "message",
          );
        }
      } catch (err) {
    ...
    _showFlashMessage: function (notifyData, type) {
      ...
      if (
        $("#elFlashMessage").is(":visible") &&
        $("#elFlashMessage").find('[data-role="newNotification"]').length
      ) {
    ...
    _updateIcon: function (type, count) {
      ...
      if (parseInt(count)) {
        ips.utils.anim.go(
          !icon.is(":visible") ? "zoomIn" : "pulseOnce",
          icon.removeClass("ipsHide"),
        );
      } else {
        icon.fadeOut();
      }
  ...
  ips.controller.register("core.front.core.lightboxedImages", {
    ...
    _initializeAttachments: function () {
      ...
      attachments.each(function () {
        ...
        if (!clone.text().length) {
          ...
          attachment.html(
            ips.templates.render("core.attachments.attachmentPreview", {
              title: title,
            }),
          );
        } else {
      ...
      ips.utils.lazyLoad.observe(this.scope.get(0), {
        loadCallback: function () {
          ips
            .getAjax()(
              ips.getSetting("baseURL") +
                "index.php?app=core&module=system&controller=ajax&do=attachmentInfo",
              {
                dataType: "json",
                data: {
    ...
    _updateAttachmentMetaData: function (attachment, response) {
      ...
      if (attachment.hasClass("ipsAttachLink_block")) {
        attachment.find(".ipsAttachLink_metaInfo").html(
          ips.templates.render("core.attachments.metaInfo", {
            size: response.size,
        ...
        attachment.attr(
          ...
          response.size +
            " - " +
            ips.pluralize(ips.getString("attachmentDownloads"), response.downloads),
        );
      }
    ...
    _nonLazyLoadInit: function (image) {
      ...
      if (
        (!image.is("img") || image.is("[data-emoticon], .ipsEmoji")) &&
        !image.hasClass("ipsImage_thumbnailed")
      ) {
        return;
      }
    ...
    _addOrUpdateWrappingLink: function (image) {
      ...
      if (
        closestLink.length &&
        closestLink.hasClass("ipsAttachLink") &&
        closestLink.hasClass("ipsAttachLink_image")
      ) {
        ...
        image.wrap(
          $(
            '<a href="' +
              ...
              '"' +
              " title='" +
              ips.getString("enlargeImage") +
              "' data-wrappedLink data-ipsLightbox data-ipsLightbox-group='" +
              self._random +
              "'></a>",
          ),
        );
...
(function ($, _, undefined) {
  ...
  ips.controller.register("core.front.core.navBar", {
    ...
    initialize: function () {
      ...
      if (
        !$("body").attr("data-controller") ||
        $("body").attr("data-controller").indexOf("core.global.customization.visualLang") == -1
      ) {
        this.setup();
      } else {
        var self = this;
    ...
    setup: function () {
      ...
      if (
        this._usingSubBars &&
        this.scope.find(".ipsNavBar_secondary > li.ipsNavBar_active").length > 1
      ) {
      ...
      if (!this._usingSubBars) {
        this.scope
          .find("#elNavigationMore_dropdown")
          .append(" <i class='fa fa-caret-down'></i>")
          .after(
            this.scope
              .find("#elNavigationMore_more_dropdown_menu")
      ...
      if (this._usingSubBars) {
        if (ips.utils.events.isTouchDevice()) {
          ...
          this.scope.hoverIntent(
            _.bind(this.intentOver, this),
            $.noop,
            '[data-role="primaryNavBar"] > li',
          );
        }
      }
      this._defaultItem = this.scope
        .find('[data-role="primaryNavBar"] > li > [data-navDefault]')
        .attr("data-navitem-id");
      this._mushAllMenus();
    },
    ...
    _makeDefaultActive: function () {
      ...
      if (link.closest('[data-role="secondaryNavBar"]').length) {
        link
          .closest('[data-role="secondaryNavBar"]')
          .closest("li")
          .addClass("ipsNavBar_active")
          .find("> a")
    ...
    _mushMenu: function (bar, widthAdjustment) {
      var self = this;
      var padding =
        parseInt(this.scope.css("padding-left")) + parseInt(this.scope.css("padding-right"));
        ...
        if (sizeIncrement + itemSize + moreMenuSize > availableSpace) {
          if (!item.attr("data-mushed")) {
            ...
            if (self._usingSubBars) {
                ...
                if (newLI.find('> [data-role="secondaryNavBar"] > li').length) {
                  ...
                  newLI.find('> [data-role="secondaryNavBar"] > li').each(function () {
                    ...
                    newDropdown.append(
                      newMenuItem
                        .append(menuContent)
                        .attr("data-originalItem", $(this).identify().attr("id")),
                    );
                  });
                  newA
                    .attr("data-ipsMenu", "")
              ...
              self.scope
                .find("#elNavigationMore_dropdown_menu")
                .append(newLI.addClass("ipsMenu_item"));
              if (newLI.find(".ipsMenu").length) {
                newLI.addClass("ipsMenu_subItems");
              }
            }
          ...
          var mushedItem = mushedParent.find(
            '[data-originalItem="' + item.identify().attr("id") + '"]',
          );
          if (mushedItem.children("a").is("[data-ipsMenu]")) {
            mushedItem
              .children("a")
              .attr("data-ipsMenu-appendTo", "#" + item.identify().attr("id"));
          }
      ...
      if (bar.is('[data-role="primaryNavBar"]')) {
        if (this._usingSubBars) {
          moreItem.toggleClass(
            "ipsHide",
            bar.find('> [data-role="navMore"] > [data-role="secondaryNavBar"] > li').length <= 1,
          );
        } else {
          moreItem.toggleClass(
            "ipsHide",
            !this.scope.find("#elNavigationMore_dropdown_menu > li").length,
          );
        }
      } else {
    ...
    _mushAllMenus: function () {
      this._mushMenu(
        this.scope.find('[data-role="primaryNavBar"]'),
        this.scope.find("#elSearch").outerWidth(),
      );
...
(function ($, _, undefined) {
  ...
  ips.controller.register("core.front.core.quickSearch", {
    ...
    closeSearch: function (e, data) {
      ...
      if (
        clickedOnTrigger ||
        $(e.target).closest("[data-ipsMenuValue]").length ||
        $(e.target).attr("id") == "elSearchField" ||
        $.contains($("#elSearchExpanded").get(0), $(e.target))
      ) {
        $("#elSearchField").focus();
        return true;
      }
    ...
    openSearch: function (e, data) {
      ...
      if (this._moreOptions === null) {
        var url =
          ips.getSetting("baseURL") +
          "index.php?app=core&module=search&controller=search&do=globalFilterOptions&exclude=" +
          $('[data-action="showMoreSearchContexts"]').attr("data-exclude");
        ips
          .getAjax()(url)
          .done(
            function (response) {
  ...
  ips.controller.register("core.front.core.reaction", {
    ...
    _updateReaction: function (response, flashMsg) {
      if (this._reactCount.hasClass("ipsReact_reactCountOnly")) {
        this._reactCount
          .find('[data-role="reactCountText"]')
          .text(response.score)
          .removeClass(
            "ipsAreaBackground_positive ipsAreaBackground_negative ipsAreaBackground_light",
          );
    ...
    clickReaction: function (e) {
      ...
      if (
        ...
        !this._singleReaction &&
        !this._reactTypeContainer.hasClass("ipsReact_types_active")
      ) {
        return;
      }
      var self = this;
      ...
      var newReaction =
        !$(e.currentTarget).closest('[data-action="reactLaunch"]').length ||
        !this._reactButton.hasClass("ipsReact_reacted");
        ...
        this._ajaxObj = ips
          ...
          .fail(function (jqXHR, textStatus, errorThrown) {
            ...
            if (
              ...
              jqXHR.responseJSON.error == "react_daily_exceeded"
            ) {
              ips.ui.alert.show({
                type: "alert",
                icon: "warn",
...
(function ($, _, undefined) {
  ...
  ips.controller.register("core.front.core.recommendedComments", {
    ...
    _doRefresh: function (newId) {
      var self = this;
      ips
        .getAjax()(this.scope.attr("data-url"))
        .done(function (response) {
    ...
    _handleResponse: function (response, newId) {
      var content = $("<div>" + response.html + "</div>").find(
  ...
  ips.controller.register("core.front.core.reputation", {
    ...
    giveReputation: function (e) {
      ...
      ips
        .getAjax()(url)
        .done(function (response) {
          var newHTML = $("<div>" + response + "</div>")
            .find('[data-controller="core.front.core.reputation"]')
            .html();
    ...
    menuOpened: function (e, data) {
      if (data.elemID == "elFullInbox" || data.elemID == "elMobInbox") {
        this._loadMenu(
          "inbox",
          ips.getSetting("baseURL") +
            "index.php?app=core&module=messaging&controller=messenger&overview=1&_fromMenu=1",
          "inbox",
        );
      } else if (data.elemID == "elFullNotifications" || data.elemID == "elMobNotifications") {
        this._loadMenu(
          "notify",
          ips.getSetting("baseURL") + "index.php?app=core&module=system&controller=notifications",
          "notify",
        );
        ...
        this._loadMenu(
          "reports",
          ips.getSetting("baseURL") +
            "index.php?app=core&module=modcp&controller=modcp&tab=reports&overview=1",
          "reports",
        );
      }
    },
